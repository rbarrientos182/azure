...
/*$consulta = "SELECT d.iddeposito, op.idoperacion AS numope, d.deposito, DATE_FORMAT(fecha, '%d/%m/%Y') AS Fecha, programadas rprogramadas, 
rutasop.rutasopt - programadas roptimizadas, clientes,  csio, FORMAT(Prom_sio,1) prosio, FORMAT(Cfisicas,1) cgepp, FORMAT(prom_fis,1) progepp, 
FORMAT(equiv,1) cequi, CONCAT(ROUND(M2C,1), ' %') M2C, capacidad, FORMAT(km,1) km FROM deposito d
LEFT JOIN (
SELECT ord.fecha, d.iddeposito, d.deposito,  op.idoperacion, ord.idruta, COUNT(DISTINCT ord.idruta) programadas, COUNT(DISTINCT nud) clientes, SUM(csio) Csio, SUM(csio)/COUNT(DISTINCT ord.idRuta) Prom_sio,
SUM(cfisicas) Cfisicas, SUM(cfisicas)/COUNT(DISTINCT ord.idRuta) Prom_fis, SUM(cequivalentes) Equiv,
ROUND(SUM(CASE WHEN cfisicas < 2 THEN 1 ELSE 0 END) * 100/COUNT(DISTINCT nud),1) AS M2C, capacidad, capcamion.km
FROM deposito d
INNER JOIN operaciones op ON op.iddeposito = d.iddeposito 
LEFT JOIN orden ord ON ord.idoperacion = op.idoperacion
INNER JOIN (
SELECT fecha, iddeposito, CONCAT(ROUND(AVG(capac),1), ' %') capacidad, SUM(km) km FROM(
SELECT fecha, d.iddeposito, ord.idruta, SUM(cfisicas) *100 / capacidad  capac, km  FROM deposito d
INNER JOIN operaciones op ON op.iddeposito = d.iddeposito
LEFT JOIN orden ord  ON ord.idoperacion = op.idoperacion
INNER JOIN ruta r ON r.idruta = ord.idruta AND r.idoperacion = ord.idoperacion
INNER JOIN unidades u ON u.idunidades = r.idunidades
WHERE fecha > CURRENT_DATE
GROUP BY fecha, d.iddeposito, ord.idruta) capc
GROUP BY iddeposito
) capcamion ON capcamion.iddeposito = d.idDeposito AND capcamion.fecha = ord.fecha
GROUP BY d.deposito
) dat ON dat.iddeposito = d.idDeposito
INNER JOIN (
SELECT d.iddeposito, COUNT(idruta) rutasopt FROM deposito d
...
) rutasop ON rutasop.iddeposito = d.iddeposito
INNER JOIN operaciones op ON op.iddeposito = d.idDeposito
ORDER BY d.deposito
LIMIT ".$inicio.", ".$fin;
