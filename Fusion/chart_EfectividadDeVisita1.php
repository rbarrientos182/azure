...
$consulta = "SELECT 
    fechaoperacion,
    (SELECT 
            CONCAT(ELT(WEEKDAY(DATE_SUB(fechaoperacion,INTERVAL $intervalo DAY)) + 1,
                                'Lunes',
                                'Martes',
                                'Miercoles',
                                'Jueves',
    ...
    FORMAT(AVG((Clientesvp / clientesprog) * 100),1) AS efectividad_visita
FROM
    resumen_ruta
WHERE
...
do{
    ...
    $cadena2 .= $row['efectividad_visita'].",";
    $cadenaDias .= '{"dia": "'.$row['dia'].'","porcentaje": '.$row['efectividad_visita'].'},';
