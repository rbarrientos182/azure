...
//preguntamos que nivel de usuario trae para mostrar los productos de acuerdo al tipo de mercado
if($nivel==2){
	...
	FROM
	    ...
	    Productos P ON Pc.sku = P.sku
	        INNER JOIN
	    usrcambios u ON u.idoperacion = pc.idoperacion
	        INNER JOIN
	    rutascambios r ON r.idoperacion = u.idoperacion
	        AND pc.idoperacion = $idoperacion
	        AND pc.estatus = 1
	        AND u.idoperacion = $idoperacion
	        AND r.idoperacion = $idoperacion
	        AND u.NumEmpleado = $NumEmpleado
	        AND pc.tmercado = r.tMercado
	        AND u.PPP = r.ruta
	ORDER BY DescripcionInterna;"
...
//preguntamos si es admin 
	...
	/** Query del catalogo de los productos por CEDIS **/
	$consulta2 = "SELECT idProductoCambio,  DescripcionInterna, Descripcion, p.sku FROM ProductosCambios Pc INNER JOIN Productos  P ON Pc.sku=P.sku WHERE idoperacion=$idoperacion AND pc.estatus = 1  ORDER BY DescripcionInterna";
	$resultado2 = $db->consulta($consulta2);
	$row2 = $db->fetch_assoc($resultado2);
}
