<div id="uploadFile">
	Subiendo Grupos Supervisores no cierre la ventana<img src="img/ajax-loaders/ajax-loader-7.gif" title="img/ajax-loaders/ajax-loader-7.gif">
</div>
<?php
if (!isset($_SESSION))
{
...
require_once('clases/class.GruposSupervisores.php');

/** obtengo la variable de sesion del deposito **/
$supervisor = new GruposSupervisores();
...
if($_POST['action']=='upload'){
	...
	if($total>0)
	{
		for($i=0;$i<$total;$i++)
		{
			...
			//echo 'archivo: ';
			//echo $archivo;
			$destino = 'grupossupervisores/'.$archivo;
			...
			if($archivo!="")
			{
				...
				if(move_uploaded_file($_FILES['fileInput']['tmp_name'], $destino))
				{
					...
					$supervisor->setArchivo($archivo);
					$mensaje = $supervisor->leerArchivo();
			...
			header('Location: sSupervisores.php?a=1&f='.$mensaje);
	...
	else
	{
		...
		header('Location: sSupervisores.php?a=0&f='.$mensaje);
	}
}
?>
