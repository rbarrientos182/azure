		...
		for($i=0;$i<$total;$i++){
			...
			if($archivo!=""){
				...
				if(move_uploaded_file($_FILES['fileInput']['tmp_name'], $destino)){
					...
					$tiempos->setArchivo($destino);
					$tiempos->setIdDeposito($iddeposito);
					$tiempos->leerArchivo();
			...
			header('Location: sTiemposO.php?a=1&f='.$archivo);
		}
	}
	else{
		header('Location: sTiemposO.php');
	}
}
?>