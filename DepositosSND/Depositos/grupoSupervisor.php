...
$consulta = "SELECT DISTINCT
    ...
    CONCAT(FORMAT(AVG((((rr.odometrofin - rr.odometroini) / kmtotal) - 1) * 100),1),'%') AS desviacion,

    FORMAT(AVG((odometrofin - odometroini) - kmtotal),1) AS Dif,
    IF(FORMAT(AVG((((rr.odometrofin - rr.odometroini) / kmtotal) - 1) * 100),
            1) > 20,
        'rRojo',
...
FROM
    ...
    FROM
        km_fijos ord
    INNER JOIN ruta ru ON ru.iddeposito = ord.no_deposito
        AND ru.idRuta = ord.ruta
    INNER JOIN horariotablero ht ON ht.iddeposito = ru.iddeposito
        AND ht.idRuta = ru.idruta
    WHERE
        ord.no_deposito = $iddeposito AND ord.dia = '$diaL') a ON rr.idruta = a.ruta
				...
				do{
					...
					FROM
					    ...
					    INNER JOIN horariotablero ht ON ht.iddeposito = ru.iddeposito
					        AND ht.idRuta = ru.idruta
					    WHERE
					        ord.no_deposito = $iddeposito AND ord.dia = '$diaL' AND ru.grupo=$grupo) a ON rr.idruta = a.ruta
					        AND rr.iddeposito = a.no_deposito
					WHERE
					    rr.iddeposito = $iddeposito
					        AND fechaOperacion = CURDATE()
					        AND tiporuta = 6  AND ((odometrofin - odometroini) -kmtotal) BETWEEN -200 AND  200
					GROUP BY grupo";
